...
for ii=iis
    ...
    plot(ps(ii).qt(:,it),ps(ii).zm/ps(ii).zi(it),base(ii).style,'Color',base(ii).color); hold on
end
...
for ii=iis
    plot(ps(ii).tl(:,it),ps(ii).zm/ps(ii).zi(it),base(ii).style,'Color',base(ii).color); hold on
end
...
for ii=iis
    plot(ps(ii).tl(:,it),ps(ii).zm/ps(ii).zi,base(ii).style,'Color',base(ii).color); hold on
end
...
ylim([0 1.2]); 
xlabel('$q(z)$ (m s$^{-1}$)','Interpreter','latex');
ylabel('$z/z_i$','Interpreter','latex');set(gca,'fontsize', fs,'fontname',fnt)
text(.0,1.1,'g)','Units','Normalized','VerticalAlignment','Top','FontSize',fs,'Interpreter','latex') %subplot label

sp8=subplot(248); %ql 
for ii=iis
    plot(ps(ii).ql(:,it),ps(ii).zm/ps(ii).zi(it),base(ii).style,'Color',base(ii).color); hold on
end
...
set(sp1,'Position',[0.05 0.52 0.12 0.44])
set(sp3,'Position',[0.05 0.1 0.29 0.29])
set(sp2,'Position',[0.22 0.52 0.12 0.44])

set(sp4,'Position',[0.4 0.52 0.12 0.44])
set(sp6,'Position',[0.4 0.1 0.29 .29])
set(sp5,'Position',[0.57 0.52 0.12 0.44])

set(sp7,'Position',[0.75 0.59 0.13 0.37])
set(sp8,'Position',[0.75 0.1 0.13 0.37])
lgd.Position=[0.895 0.35 0.1 0.33];
