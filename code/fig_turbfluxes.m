...
%     plot(tke,ps(ii).zm./zi,base(ii).style,'Color',gnrl.cols(ii,:),'LineWidth',lw,'MarkerIndices',1:20:131); hold on
% end
...
sp3=subplot(243); % q'w' (momentum flux
for ii=1:11
    it3=find(ps(ii).time==3.75*3600)+1;
    it4=find(ps(ii).time==4*3600);
...
for ii=iis
    ...
    plot(buoy.*1e4,ps(ii).zm./zi,base(ii).style,'Color',gnrl.cols(ii,:),'LineWidth',lw,'MarkerIndices',1:20:131); hold on
end
ylabel('$z/z_i$','Interpreter','latex'); ylim([0 1.05]);
...
plot(tw./dn0./cp*100,ps(ii).zm./zi,base(ii).style,'Color',gnrl.cols(ii,:),'LineWidth',lw,'MarkerIndices',1:20:131); hold on
end
...
for ii=iis
    ...
    plot(qw./dn0./Lv*1e3*100,ps(ii).zm./zi,base(ii).style,'Color',gnrl.cols(ii,:),'LineWidth',lw,'MarkerIndices',1:20:131); hold on
end
